...
def _detect_feishu():
    """规则一：启动时自动检测飞书环境变量，直接告知结果，不询问。"""
    if is_feishu_configured():
        print("✅ 飞书已配置（Base Token / Wiki 链接就绪），分析结果将直接入库。")
    else:
        print("ℹ️ 未检测到飞书配置，结果将本地暂存到 pending_results.json，")
        print("   配置 FEISHU_BASE_TOKEN（裸 token 或 Wiki 链接）+ FEISHU_TABLE_ID 后重跑即可自动上传。")
...
def phase4_store(completed, failed):
    """completed: list of (url, owner_repo, stars, AnalysisResult)
    failed:   list of (url, owner_repo, error)
    """
            ...
            for item in pending:
                ...
                if write_record_with_retry(upload_fields):
                    ...
                    items.append(ReportItem(
                        item.get("fldvVvQNRR", ""), owner_repo, "success"))
                else:
                    items.append(ReportItem(
                        item.get("fldvVvQNRR", ""), owner_repo, "failed",
                        error_reason="Feishu write failed"))
        # 2) 上传本次新结果
        for url, owner_repo, stars, result in completed:
            fields = result.to_feishu_fields(
                owner_repo.split("/")[-1], url, stars)
            if write_record_with_retry(fields):
                append_to_imported_list(owner_repo)
                items.append(ReportItem(url, owner_repo, "success",
                                        project_type=result.project_type))
            else:
                items.append(ReportItem(url, owner_repo, "failed",
                                        error_reason="Feishu write failed"))
    else:
        print(f"\n📦 阶段四：未检测到飞书配置，本地暂存")
        for url, owner_repo, stars, result in completed:
    ...
    return {"items": items, "local_saved": local_saved}
...
def main():
    ...
    report_data = phase4_store(completed, failed)
